function ureal = stokes_dlp_pressure_grad_real_ds(xsrc, ysrc, xtar, ytar,...
        ...
        for jpx = -1:1 %Go through a layer of boxes in the x direction
            ...
            for jpy = -1:1 %Go through a layer of boxes in the y direction
                ...
                if abs(r) < 1e-13
                    ...
                    [utmp1, utmp2] =  stokeslet_real_pressure_grad_sum(r1,r2,...
...
ureal = [ureal1; ureal2] / (2*pi);

end

function [u1, u2] = stokeslet_real_pressure_grad_sum(r1, r2, n1, n2, f1, f2, xi)
...
u1 = exp(-xi*xi*r^2)*(2*xi*xi*(r1*fdotn+f1*rdotn+n1*rdotf-2*xi*xi*r1*rdotn*rdotf)/r^2 ...
    +2*(-4*xi*xi*r1*rdotn*rdotf+r1*fdotn+f1*rdotn+n1*rdotf)/r^4 - 8*r1*rdotn*rdotf/r^6);
u2 = exp(-xi*xi*r^2)*(2*xi*xi*(r2*fdotn+f2*rdotn+n2*rdotf-2*xi*xi*r2*rdotn*rdotf)/r^2 ...
    +2*(-4*xi*xi*r2*rdotn*rdotf+r2*fdotn+f2*rdotn+n2*rdotf)/r^4 - 8*r2*rdotn*rdotf/r^6);
