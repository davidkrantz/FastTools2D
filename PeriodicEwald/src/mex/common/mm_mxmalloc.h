...
_mm_mxMalloc (size_t size, size_t align)
{
  ...
  if (align & (align - 1))
    {
      mexErrMsgTxt("Error in _mm_mxmalloc : alignment not power of two.");
      errno = EINVAL;
      return ((double*) 0);
    }

  if (size == 0) {
    mexErrMsgTxt("Error in _mm_mxmalloc : cannot allocate zero bytes.");    
    return ((double *) 0);
  }
 /* Assume malloc'd pointer is aligned at least to sizeof (void*).
  ...
  malloc_ptr = static_cast<double*>(mxMalloc (size + align));
  if (!malloc_ptr)
    return ((double *) 0);
...
_mm_mxCalloc (size_t size, size_t typesize ,size_t align)
{
  ...
  if (align & (align - 1))
    {
      mexErrMsgTxt("Error in _mm_mxcalloc : alignment not power of two.");
      errno = EINVAL;
      return ((double*) 0);
    }

  if (size == 0) {
      mexErrMsgTxt("Error in _mm_mxcalloc : cannot allocate zero bytes.");    
    return ((double *) 0);
  }
 /* Assume malloc'd pointer is aligned at least to sizeof (void*).
    If necessary, add another sizeof (void*) to store the value
  ...
  calloc_ptr = static_cast<double*>(mxCalloc (size + align/typesize,typesize));
  if (!calloc_ptr)
    return ((double *) 0);
