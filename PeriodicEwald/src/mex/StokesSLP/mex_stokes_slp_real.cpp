...
void Assign(double *psrc, double *ptar, double len_x, double len_y, int nsrc, 
    ...
    for(j = 0;j<nsrc;j++) {
        ...
        //mexPrintf("source %d: x = %3.3f, y = %3.3f, x box: %d, y box %d\n", j, psrc[2*j], psrc[2*j+1], box_x, box_y);
        //mexPrintf("%3.3f\n", nside_x*(psrc[2*j]/len_x+0.5) );
    ...
    for(j = 0;j<ntar;j++) {
        ...
        //mexPrintf("target %d: x = %3.3f, y = %3.3f, x box: %d, y box %d\n", j, ptar[2*j], ptar[2*j+1], box_x, box_y);
        //mexPrintf("%3.3f\n", nside_x*(ptar[2*j]/len_x+0.5) );
        if(box_x < 0) box_x = 0;
        if(box_x >= nside_x) box_x = nside_x-1;
        if(box_y < 0) box_y = 0;
        if(box_y >= nside_y) box_y = nside_y-1;
    ...
    for(j=1;j<number_of_boxes+1;j++) {
        ...
        particlenum_in_box_src[j] = box_offsets_src[j] = isrc;
        itar += ntargets_in_box[j-1];
        particlenum_in_box_tar[j] = box_offsets_tar[j] = itar;
    }
