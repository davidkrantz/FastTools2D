...
c       
c        ifprint = 1
c        if(ifprint .eq. 1) call prin2('epsfmm=*',epsfmm,1)
c        ifprint = 1
c
c     set criterion for box subdivision (number of sources per box)
c
c        if( iprec .eq. -2 ) nbox=3
c        if( iprec .eq. -1 ) nbox=5
c        if( iprec .eq. 0 ) nbox=8
c        if( iprec .eq. 1 ) nbox=10
c        if( iprec .eq. 2 ) nbox=15
c        if( iprec .eq. 3 ) nbox=20
c        if( iprec .eq. 4 ) nbox=25
c        if( iprec .eq. 5 ) nbox=45
c        if( iprec .eq. 6 ) nbox=nsource+ntarget
        if( iprec .eq. -2 ) nbox=15
        if( iprec .eq. -1 ) nbox=15
        if( iprec .eq. 0 ) nbox=15
        if( iprec .eq. 1 ) nbox=20
        if( iprec .eq. 2 ) nbox=25
        if( iprec .eq. 3 ) nbox=25
        if( iprec .eq. 4 ) nbox=30
        if( iprec .eq. 5 ) nbox=50
        if( iprec .eq. 6 ) nbox=nsource+ntarget

c
c
c        ifprint = 1
c        if(ifprint .eq. 1) call prinf('points per box=*',nbox,1)
c        ifprint = 1
c
c
...
c       
c        ifprint = 0
c        if(ifprint .eq. 1) call prin2('scale=*',scale,nlev+1)
c        ifprint = 1
c       
c
...
c      
c        ifprint = 0
c        if(ifprint .eq. 1) call prinf(' lused7 is *',lused7,1)
c        ifprint = 1
c
...
c        ifprint = 0
c        if (ifprint.eq.1) 
c     $     call prin2('in lfmm2dpart, bsize(0)=*',
c     $     abs(bsize(0)),1)
c        ifprint = 1
c
c        ifprint = 0
c        if (ifprint.eq.1) call prin2('bsize=*',bsize,nlev+1)
c        ifprint = 1
c        ifprint = 1
c        if (ifprint.eq.1) call prinf('nterms=*',nterms,nlev+1)
c        ifprint = 0
c
c       
...
c        call prinf('finished reordering=*',ier,1)
c        call prinf('ier=*',ier,1)
c        call prinf('nboxes=*',nboxes,1)
c        call prinf('nlev=*',nlev,1)
c        call prinf('lused7=*',lused7,1)
c        endif
c        ifprint=0
c
c
...
c
c        ifprint = 0
c        if(ifprint .eq. 1) call prinf(' lmptot is *',lmptot,1)
c        ifprint = 1
c       
        irmlexp = 1
...
c
c        ifprint=0
c        if(ifprint .eq. 1) call prinf('lwlists=*',lwlists,1)
c        ifprint=1
c        ifprint=0
c        if(ifprint .eq. 1) then
c        call prinf('lused total=*',lused7,1)
c        call prinf('lused total(k)=*',lused7/1000,1)
c        call prinf('lused total(M)=*',lused7/1000000,1)
c        endif
c        ifprint=1
c       
c        ifprint=0
...
c
c        if (ifprint .ge. 2) 
c     $     call prinf('nterms_eval=*',nterms_eval,4*(nlev+1))
c
...
c        ifprint=0
c        if(ifprint .ge. 1) 
c     $     call prinf('=== STEP 1 (form mp) ====*',i,0)
c        ifprint=1
c        call cpu_time(t1)
c        t1=second()
c        t1=omp_get_wtime()
...
c        if (ifprint .ge. 2) then
c           call prinf('npts=*',npts,1)
c           call prinf('isource=*',isource(box(9)),box(10))
c        endif
        endif
c
...
c         ifprint = 0
c         if(ifprint .ge. 1)
c     $      call prinf('=== STEP 2 (form lo) ====*',i,0)
c         ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c         if (nlist .gt. 0) then 
c            if (ifprint .ge. 2) then
c               call prinf('ibox=*',ibox,1)
c               call prinf('list3=*',list,nlist)
c            endif
c         endif
c
c       ... prune all sourceless boxes
...
c        ifprint = 0
c        if(ifprint .ge. 1)
c     $     call prinf('=== STEP 6 (eval mp) ====*',i,0)
c        ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c        ifprint = 0
c        if(ifprint .ge. 1)
c     $     call prinf('=== STEP 7 (eval lo) ====*',i,0)
c        ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c        ifprint = 0
c        if(ifprint .ge. 1)
c     $     call prinf('=== STEP 8 (direct) =====*',i,0)
c        ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c
c        ifprint=1
c        if (ifprint .ge. 1) call prin2('timeinfo=*',timeinfo,8)
c       
c        d=0
c        do i=1,8
c        d=d+timeinfo(i)
c        enddo
c       
c        if (ifprint .ge. 1) call prin2('sum(timeinfo)=*',d,1)
c        ifprint=0
c
c        ifprint=0
c        if (ifprint .ge. 1) then
c        call prinf('nboxes=*',nboxes,1)
c        call prinf('nsource=*',nsource,1)
