...
c        if (ifprint.eq.1) call prinf('lused7=*',lused7,1)
...
c       
c        if (ifprint.eq.1) 
c     $       call prin2('after d2tstrcr, center=*',center,2)
c        if (ifprint.eq.1) 
c     $       call prin2('after d2tstrcr, size=*',size,1)
c        if (ifprint.eq.1) 
c     $       call prinf('after d2tstrcr, nlev=*',nlev,1)
c        if (ifprint.eq.1) 
c     $       call prinf('after d2tstrcr, nbox=*',nbox,1)
c        if (ifprint.eq.1) 
c     $       call prinf('after d2tstrcr, laddr=*',laddr,2*(nlev+1))
c
...
c        ifprint = 0
c         if (ifprint .ge. 1) 
c     $     call prinf('=== STEP 3 (merge mp) ===*',i,0)
c        ifprint=1
c         t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c            if (ifprint .ge. 2) then
c               call prin2('center0=*',center0,2)
c            endif
c
            do 2100 i = 1,4
               jbox = box(4+i)
...
c               call prin2('center1=*',center1,2)
c               endif
               level1=box1(1)
...
c        ifprint = 0
c        if (ifprint .ge. 1) 
c     $     call prinf('=== STEP 4 (mp to lo) ===*',i,0)
cc        ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c        ifprint = 0
c        if (ifprint .ge. 1) 
c     $     call prinf('=== STEP 5 (split lo) ===*',i,0)
c        ifprint = 1
c        t1=second()
c        call cpu_time(t1)
c        t1=omp_get_wtime()
...
c                  endif
     ...
     1   	       nterms(level1))
 5100          continue
c               if (ifprint .ge. 2) call prinf('=============*',x,0)
            endif
        endif
c
        if (nkids .ne. 0) then
            level=box(1)
c            if (level .ge. 2) then
c               if( ifprint .ge. 2 ) then
c                  call prinf('ibox=*',ibox,1)
c                  call prinf('box=*',box,15)
c                  call prinf('nkids=*',nkids,1)
