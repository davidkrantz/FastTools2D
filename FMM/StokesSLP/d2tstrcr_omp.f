...
c        t2=second()
cC$        t2=omp_get_wtime()
c        call prin2('time in d2tallbem=*',t2-t1,1)
c
...
c
c        call prinf('lused: ccenters(k)=*', (lcenters+lcorners)/1000,1)
c        call prinf('lused(k)=*', (iwlists)/1000,1)
...
c        t2=second()
cC$        t2=omp_get_wtime()
c        call prin2('time in d2tlsts=*',t2-t1,1)
c
        lused777=lused+iwlists
ccc        call prinf('after d2tlsts, ier=*',ier,1)
c        call prinf('lused(k)=*', (lused777)/1000,1)
c
...
c
c        call prin2('after d2tstrcr, center0=*',center0,3)
c
        size0=corners0(1,3)-corners0(1,1)
c        call prin2('after d2tstrcr, size0=*',size0,1)
c
        nlev0=w(9)
c        call prinf('after d2tstrcr, nlev0=*',nlev0,1)
c
        nbox0=w(8)
c        call prinf('after d2tstrcr, nbox0=*',nbox0,1)        
c
c        call prinf('after d2tstrcr, laddr0=*',w(100),2*(nlev0+1))
c
        return
        end
c
...
c
        lused=w(iilists)+w(inumele)*3+10
        ntypes7=w(intypes)
c         call prinf('in d2tlinkinfo, lused=*',lused,1)
c         call prinf('in d2tlinkinfo, ntypes7=*',ntypes7,1)
c         call prinf('in w(inumele)=*',w(inumele),1)
        do 6200 i=1,ntypes7
        nums(i)=w(inums(i))
 6200 continue
        return
